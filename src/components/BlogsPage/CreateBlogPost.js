...
const fieldBase = 'w-full rounded-xl border border-gray-200 bg-white/80 text-gray-900 placeholder-gray-400 ring-1 ring-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed transition-shadow';
...
export default function CreateBlogPost({ onSave, initialData = {}, isModal = false, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Append all scalar form fields, excluding complex objects and files
      Object.entries(formData).forEach(([key, value]) => {
        if (key === 'tags') {
          // Ensure tags are always an array before stringifying
          formDataToSend.append('tags', JSON.stringify(value));
        } else if (key === 'keywords') { // NEW: Append keywords similarly
          formDataToSend.append('keywords', JSON.stringify(value));
        } else if (!['courseSections', 'blogImage', 'bannerImage'].includes(key) && value !== null && value !== undefined) {
          // Append other simple fields
          formDataToSend.append(key, value);
        }
      });

      // 2. Append the main blog image and banner image if they exist
      if (formData.blogImage) formDataToSend.append('image', formData.blogImage);
      if (formData.bannerImage) formDataToSend.append('bannerImage', formData.bannerImage);

      // 3. Filter out any incomplete course sections first to ensure data integrity
      const validCourseSections = formData.courseSections
        .filter(s => s.name?.trim() && s.description?.trim() && s.url?.trim());

      // 4. Build the courses JSON array from the valid sections
      const coursesJSON = validCourseSections.map(s => {
        const courseData = {
          name: s.name.trim(),
          description: s.description.trim(),
          courseUrl: s.url.trim(),
        };
        // If an image exists from the server and wasn't replaced, include its URL
        if (s.imagePreview && !s.imageFile) {
          courseData.courseImage = s.imagePreview;
          courseData.courseImagePublicId = s.imagePublicId ?? null;
        }
        return courseData;
      });
      formDataToSend.append('courses', JSON.stringify(coursesJSON));

      // 5. Append course image files ONLY from the corresponding valid sections
      validCourseSections.forEach(s => {
        if (s.imageFile) {
          formDataToSend.append('courseImages', s.imageFile);
        }
      });

      // 6. Add author fallback if not provided
      if (!formData.authorName && user?.username) {
        formDataToSend.set('author', user.username);
      }
      ...
      if (!isEditMode) {
        ...
        setExistingImageUrl(null);
        setExistingBannerUrl(null);
        // Reset common course image
        setCommonCourseImage({ file: null, preview: null });
        fetchAvailableTags(); // Refresh tags list
        fetchAvailableKeywords(); // NEW: Refresh keywords list
      }
    } catch (err) {
    ...
    <div className={isModal ? 'w-full min-h-screen bg-gray-50' : 'w-full'}>
      <div className={isModal ? 'w-full mx-auto' : 'w-full p-4 md:p-6 lg:p-8'}>
        <div className="w-full">
            ...
            <div className="bg-white/95 backdrop-blur rounded-2xl shadow-lg border border-gray-100">
              ...
              <form id="blog-form" onSubmit={handleSubmit} className="p-4 md:p-6 lg:p-8 space-y-6 pb-28 sm:pb-0">
                ...
                <div className="group">
                  ...
                  {/* Rich Text Editor */}
                  <div className="rounded-xl border border-gray-200 bg-white/80 ring-1 ring-gray-200 focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2 transition-shadow overflow-hidden">
                    <ReactQuill
                      ref={quillRef}
                      theme="snow"
          ...
          {!isModal && (
            <div className="fixed right-0 top-0 h-screen w-96 overflow-y-auto bg-white shadow-lg z-50">
              <div className="bg-white p-6 h-full">
                ...
                <div className="bg-gray-50 rounded-xl p-4 min-h-48 flex flex-col">
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <p className="text-xs text-gray-500 text-center mt-1">Featured Image</p>
                    ...
                    <p className="text-xs text-gray-500 text-center mt-1">Banner Image</p>
                  </div>
      ...
      <style jsx global>{`
        ...
        /* Hidden default Quill toolbar since we use custom sticky toolbar */
        .ql-toolbar {
